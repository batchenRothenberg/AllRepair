...
{
  out << "I don't think we have a footer...\n"; //bat
  out << "\n";
...
std::string smt2_convt::convert_identifier(const irep_idt &identifier)
{
  ...
  for(std::size_t i=0; i<identifier.size(); i++)
  {
    ...
    case '&': // we use the & for escaping
      result+='&';
      result+=i2string(ch);
      result+=':'; //bat (instead of ';' because it is confused with a comment)
      break;
...
void smt2_convt::set_to(const exprt &expr, bool value, int group)
{
  ...
  {
    forall_operands(it, expr)
      set_to(*it, true, group); //bat added group
    return;
  }
  ...
  {
    forall_operands(it, expr)
      set_to(*it, false, group); //bat added group
    return;
  }
  ...
  {
    assert(expr.operands().size()==1);
    return set_to(expr.op0(), !value, group); //bat added group
  }
*/
  out << "\n";
...
/*
        ...
        //bat
        out << "; set_to true (equal) bat\n";
        out << "(declare-const |" << smt2_identifier << "| ";
        convert_type(equal_expr.lhs().type());
        out << ")" << "\n";
