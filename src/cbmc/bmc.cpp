...
void bmct::do_conversion()
{
  ...
  // convert SSA
  equation.convert(prop_conv,options); //bat added options

  //equation.output(std::cout);
  //((smt2_dect &)prop_conv).printstring();
...
bmct::run_decision_procedure(prop_convt &prop_conv)
{
  ...
  status() << "BBBRunning " << prop_conv.decision_procedure_text() << eom; //batchen
  ...
  const goto_functionst &goto_functions)
{
  ...
  try
  {
    // get unwinding info
    setup_unwind();
	  //std::cout<<"lalalalala before: "<<equation<<std::endl; //bat

    // perform symbolic execution
    symex(goto_functions);
	  //std::cout<<"lalalalala after: "<<equation<<std::endl; //bat
