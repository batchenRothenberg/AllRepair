...
marco(){
	...
	if [[ $REPAIROUT -eq 1 ]]; then
		...
		../python/batmarco.py ${out_name_no_extension}.gsmt2 ${TIMEOUT+"-T"} $TIMEOUT ${SIZELIMIT+"-k"} $SIZELIMIT ${PROGRAMLIMIT+"-l"} $PROGRAMLIMIT --smt -v -s -a --smus &> repair_out/${out_name_no_extension}.rout
	else
		../python/batmarco.py ${out_name_no_extension}.gsmt2 ${TIMEOUT+"-T"} $TIMEOUT ${SIZELIMIT+"-k"} $SIZELIMIT ${PROGRAMLIMIT+"-l"} $PROGRAMLIMIT --smt -v -s -a --smus
	fi
}
...
echo "UNWIND=$UNWIND FUNCTION=$FUNCTION MUTATION=$MUTATION NOMUT=$NOMUT TIME=$TIMEOUT SIZE=$SIZELIMIT PROG=$PROGRAMLIMIT"
echo "FILES=$FILES"
echo "DIRS=$DIRS"
